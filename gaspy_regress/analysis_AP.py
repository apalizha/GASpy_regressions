...
from gaspy import gasdb, atoms_operators, utils
...
def add_stoch_ratio_to_doc(doc):
    doc['stoichiometry'] = atoms_operators.get_stoich_from_mpid(doc['mpid'])
    elem_nums = list(doc['stoichiometry'].values())
    doc['ratio'] = elem_nums[0]/sum(elem_nums)
    return doc


def create_gridplot(adsorbate, targets, filename, low_coverage=False, hovertext_labels=None):
    '''
    ...
    Args:
        ...
        low_coverage        Types of docs we are pulling, false = get_adsorption_docs,
                            true = get_low_coverage_dft_docs.
    ...
    if hovertext_labels is None:
        hovertext_labels = {'coordination', 'energy', 'stoichiometry', 
                            'mpid', 'miller'}

    # Get and preprocess all the documents we have now
    if low_coverage is False:
        extra_projections = {'atoms': '$atoms',
                         'date': '$calculation_dates.slab+adsorbate'}
        all_docs = gasdb.get_adsorption_docs(adsorbate, extra_projections)
    elif low_coverage is True:
        all_docs = gasdb.get_low_coverage_dft_docs(adsorbate)
    
    #get only bimetallic docs and get stoichometry and ratio
    with open(utils.read_rc('gasdb_path') + 'mp_comp_data.pkl', 'rb') as file_handle:
        composition_by_mpid = pickle.load(file_handle)
    for doc in all_docs:
        try:
            doc['composition'] = composition_by_mpid[doc['mpid']]  
        except KeyError:
            doc['composition'] = list(atoms_operators.get_stoich_from_mpid(doc['mpid']).keys())
            
    bimetallic_docs = [doc for doc in all_docs if len(doc['composition']) <= 2]
    bimetallic_docs = utils.multimap(add_stoch_ratio_to_doc, bimetallic_docs, 
                                    processes=64, n_calcs=len(bimetallic_docs))
    
    all_elements = {element for doc in bimetallic_docs
                    for element in doc['composition']}
    
    # Organize all of our documents according to their bimetallic composition
    docs_by_comp = defaultdict(list)
    for doc in bimetallic_docs:
    ...
    # Save it online
    plotly.sign_in(**utils.read_rc('plotly_login_info'))
